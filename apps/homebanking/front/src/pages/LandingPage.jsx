...
const PRODUCTOS = [
  { icon: PiggyBank, color: '#D11218', titulo: 'Cuenta de Ahorros', desc: 'Maneja tu dinero sin costo de mantenimiento y gana intereses todos los días.' },
...
const BENEFICIOS = [
  ...
  { icon: MapPin, titulo: 'Cobertura nacional', desc: 'Presencia en agencias y cajeros a nivel nacional.' },
]

const PROMO_SLIDES = [
  { titulo: '¡Gana S/50 SIN sorteos!', texto: 'Abre tu primera Cuenta Digital y gana' },
  { titulo: 'Tipo de cambio preferencial', texto: 'Para clientes con Cuenta Digital activa' },
  { titulo: 'Transferencias 100% gratis', texto: 'Entre cuentas Scotiabank, sin límite de monto' },
  { titulo: 'Yape y Plin al instante', texto: 'Cobra y paga sin comisiones desde tu celular' },
]

export default function LandingPage() {
  const navigate = useNavigate()
  const [activePromo, setActivePromo] = useState(0)
  const slide = PROMO_SLIDES[activePromo]
  ...
  return (
    <div className="lp-page">
      ...
      <section className="lp-hero">
        <div className="lp-hero-inner">
          <div className="lp-hero-text">
            <span className="lp-hero-tag">Banca Digital · Scotiabank</span>
            <h1>Tu banco digital, <br />siempre a tu lado</h1>
            <p>
      ...
      {/* ===== BANNER PROMOCIONAL (split rojo + visual, carrusel de puntos) ===== */}
      <section className="sb-promo-banner">
        <div className="sb-promo-banner-text">
          <div className="sb-promo-banner-text-inner">
            <h3>{slide.titulo}</h3>
            <p>{slide.texto}</p>

            <div className="sb-promo-feature">
              <Banknote size={26} />
            ...
            <div className="sb-promo-banner-actions">
              <button className="sb-promo-pill" onClick={() => navigate('/login')}>Ábrela aquí</button>
              <button className="sb-promo-pill" onClick={() => navigate('/login')}>Conoce más</button>
            </div>

            <div className="sb-promo-dots">
              {PROMO_SLIDES.map((_, i) => (
                <button
                  key={i}
                  className={`sb-promo-dot ${activePromo === i ? 'active' : ''}`}
        ...
        <div className="sb-promo-banner-visual">
          <Building2 size={140} color="#D11218" strokeWidth={1} />
        </div>
      </section>
      ...
      <section className="lp-section" id="beneficios">
        <div className="lp-section-head">
          <h2>¿Por qué Scotiabank?</h2>
          <p>Una banca cercana, segura y pensada para el Perú.</p>
        </div>
